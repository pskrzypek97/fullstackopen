...
const resolvers = {
	Query: {
		...
		allBooks: (root, args) => {
			const findBooksByGenre = () =>
				books.filter((b) => b.genres.includes(args.genre));

			const findBookByAuthor = () =>
				books.filter((b) => b.author === args.author);

			if (args.author) return findBookByAuthor();

			if (args.genre) return findBooksByGenre();

			if (args.author && args.author)
				return findBookByAuthor().filter((b) => b.genres.includes(args.genre));

			return books;
		},
		allAuthors: () => {
			const modifiedAuthors = authors.map((a) => {
				const authorsBooks = books.filter((b) => b.author === a.name);
